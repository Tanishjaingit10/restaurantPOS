...
const Pos = () => {
  ...
  const custList = async (e) => {
    setCust(
      customers
        ...
        .map((option) => {
          return (
            <li key={option.contact}
              className="flex flex-row text-black p-2 relative cursor-pointer"
              onClick={() => {
                setCustomer({
                  name: option.name,
            ...
            >
              ...
              <a href={`/customerDetails/${option.contact}`}>
                <i className="fas fa-arrow-right absolute right-0 p-2"></i>
              </a>
            </li>
          );
        })
              ...
              if (option.status === "Free")
                return (
                  <li key={option.number}>
                    <button
                      className="py-2 border-b-2 border-t-2 border-black w-full text-left"
                      onClick={handleTable}
                      name="category"
                      value={option.number}
                    >
  ...
  return (
    <div className="flex flex-row h-full">
      <div className="w-3/5 border-r-2 border-primary h-full shadow-2xl">
        <nav className="bg-primary p-2 mt-0 h-auto top-0 text-2xl text-white font-roboto font-semibold">
          <div className="flex flex-row px-6 justify-items-center">
            <div className=" justify-center md:justify-start text-white py-2">
              <a href="/home">
            ...
            <div className="flex flex-row w-full mx-24 relative">
              <ul className=" text-white text-left">
                ...
                >
                  {payment.orderType}
                  <span>
                    <i className="fas fa-chevron-down ml-8 cursor-pointer"></i>
                  </span>
                </li>
                {list ? (
                  <ul className="absolute bg-primary p-2 text-left text-xl">
                    <li
                      className="border-b-2 border-white py-2 cursor-pointer"
                      onClick={() => {
                        showPop(!pop);
                      }}
                    >
                      Take Away-Ordered Online
                    </li>
                    <li
                      className="border-b-2 border-white py-2 cursor-pointer"
                      onClick={() => {
                    ...
                    >
                      Dine In Ordered Online
                    </li>
                  </ul>
                ) : null}
              </ul>
              <div
                className="ml-10 text-center p-2 cursor-pointer"
                onClick={() => {
                  showCust(!cust);
                }}
              >
              ...
              {cust ? (
                <ul className="absolute z-30 top-10 right-0 bg-white mt-4 border-2 shadow-lg w-2/3 font-thin text-lg">
                  <li className="bg-primary flex flex-row">
                    <input
                      value={search}
                      onChange={(e) => setSearch(e.target.value)}
                      type="text"
                      className="bg-lightprimary py-2 w-full focus:outline-none text-black pl-4"
                    />
                    <i className="fas fa-search p-2"></i>
                  </li>
                  {Cust}
                  <li className="bg-green py-2 text-center">
                    <a href="/newCustomer">+ New Customer</a>
                  </li>
                </ul>
              ) : null}

              <div className="absolute text-center py-2 right-0">
                <i
                  onClick={() => {
                    setCustomer([]);
                  }}
                  className="fas fa-trash-alt ml-10 cursor-pointer"
                ></i>
              </div>
            </div>
          </div>
      ...
      {pop && (
        <div className="bg-primary absolute top-16 left-40 p-20">
          <div
            className=" absolute top-0 right-4 text-center cursor-pointer"
            onClick={() => {
              showPop(!pop);
            }}
          >
            <span className=" text-white text-center object-center text-xl">
              x
            </span>
          </div>
          <div className="flex flex-col text-white space-y-2 font-bold w-96 text-xl">
            ...
            <button className="bg-white text-primary py-2 font-bold">
              Search
            </button>
          </div>
        </div>
      ...
      {open && (
        <div className="bg-primary absolute top-16 left-40 p-20 z-30">
          <div
            className=" absolute top-0 right-4 text-center cursor-pointer"
            onClick={() => {
              setOpen(!open);
          ...
          <div className="flex flex-col text-white space-y-2 font-bold w-96 text-xl">
            ...
            <ul className="bg-white text-black font-normal border-l-2 border-r-2 border-black">
              <li
                className="py-2 border-b-2 border-t-2 border-black"
                onClick={openTable}
              >
                Table {payment.table ? payment.table : "No."}
              </li>
            ...
            <button
              className="bg-white text-primary py-2 font-bold"
              onClick={orderSearch}
            >
              Search
            </button>
